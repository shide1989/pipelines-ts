...
> Alpha — extensively tested via integration and E2E tests, but not yet stable.
> Source & full docs: [github.com/shide1989/pipelines-ts](https://github.com/shide1989/pipelines-ts)
...
**pipelines-ts** is the small, self-hosted version. PostgreSQL as the durable queue, `Proxy` + `AsyncLocalStorage` to intercept step execution at runtime — no compiler needed. Drop it into any TypeScript project that already has a Postgres connection.
...
**v0.5.0** — core runtime complete (durable steps, sleep, retry, replay, multi-worker liveness).
...
Requires PostgreSQL 13+ and Node.js (or Bun). The runtime ships as ESM, built with plain `tsc` — no bundler.
...
const steps = checkpoint({
  ...
  checkInference: async ({ jobId }: { jobId: string }) => {
    return await openai.batches.retrieve(jobId);
  },
...
```typescript
await setup(client);                    // reads the bundled schema.sql
